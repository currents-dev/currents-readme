...
For [Detox](../../../getting-started/other-frameworks/jest/detox-+-jest.md) runs, starting with `1.11.0-beta.0`, the command also uploads the videos, screenshots and logs that Detox recorded, attached to the test attempt they belong to, and `detox.trace.json`. Run it after `detox test` exits.
