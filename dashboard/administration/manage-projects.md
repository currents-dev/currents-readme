...
Every tab shows who last saved the configuration and when — or **Not yet published for your organization** if no layout has been saved yet. **Done** in the header leaves the editor and returns to the Projects page.
...
Hovering a row reveals its drag handle (the grip on the left). Drag and drop then changes the structure:
...
The **⋯** menu on a folder header, revealed on hover, offers:

* **Rename** the folder.
* An **icon** and a color for it (the **Icons** tab).
* **Labels** attached to the folder itself (the **Labels** tab).
...
The **⋯** menu on a project row offers:

* An **icon** and an **accent** color, shown wherever the project appears — the sidebar, the project switcher and the project lists.
* Toggles for the project's **labels**.
* **Manage project**, which opens the project's settings page.
...
Archived projects are hidden by default. **Show archived** in the header brings them into the tree so they can be re-ordered or placed into folders; archived projects are marked with an `ARCHIVED` badge. See [Archive and Unarchive Projects](../projects/archive-and-unarchive-projects.md).

## Project Labels

Labels are a shared vocabulary for an organization's projects — for example `team-web`, `nightly` or `critical`. They are defined once per organization on the **Project Labels** tab and can then be attached to both projects and folders from the **Project Structure** tab.
...
The **New Project** form has a **Directory** field for placing the project straight into an existing folder, so it does not have to be moved afterwards. Like every other structure change, the placement applies to the whole organization. Folders themselves can only be created, renamed, re-ordered and deleted in Manage Projects.

Moving a project that already exists is done by dragging it in the **Project Structure** tab.
