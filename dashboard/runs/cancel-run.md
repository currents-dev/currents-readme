---
description: Cancelling CI tests runs
---
...
A cancelled CI job stops reporting mid-run, whether you cancelled it yourself or a newer commit superseded it. The run stays in progress until it hits the project's [run-timeouts.md](run-timeouts.md "mention"). Add a step that cancels the run when the job is cancelled:
...
Pass `project-id` and `ci-build-id` as well to identify the run by its [ci-build-id.md](../../guides/parallelization-guide/ci-build-id.md "mention") instead. Use this form when the workflow records under a CI Build ID of its own:
