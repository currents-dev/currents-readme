...
{% hint style="info" %}
For **non-orchestrated** Playwright sharding (`--shard`), `@currents/cmd` is **not required** for reruns to work: `--last-failed` needs the prior run's outputs (for example `.last-run.json`). The [Last Failed GitHub Action](https://github.com/currents-dev/playwright-last-failed) can supply that data without listing `@currents/cmd` in the project—it installs and runs the CLI globally when the package is absent—but adding `@currents/cmd` as a dev dependency is still recommended so the CLI version is pinned by the lockfile (see [re-run-failed-only-tests.md](../../getting-started/ci-setup/github-actions/re-run-failed-only-tests.md "mention")). Pipelines that invoke `currents cache` or other `currents` commands directly should install `@currents/cmd` as shown in those guides.
{% endhint %}
